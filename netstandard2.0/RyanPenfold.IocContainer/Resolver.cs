...
namespace RyanPenfold.IocContainer
{
    public class Resolver
    {
        ...
        static Resolver()
        {
            ConfigurationSettings = SettingsProvider<ConfigurationSettings>.GetSection(MethodBase.GetCurrentMethod()?.DeclaringType?.Assembly.GetName().Name);
        }
        ...
        public static IServiceProvider BuildContainer(IServiceCollection services = null)
        {
            ...
            foreach (var component in ConfigurationSettings.Components)
            {
                if (component == null)
                {
                    continue;
